...
namespace CoreWCF.Xml
{
    ...
    public abstract class XmlDictionaryReader : XmlReader
    {
        ...
        {
            if (localNames == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(localNames));
        ...
        protected string ReadString(int maxStringContentLength)
        {
            ...
            if (this.NodeType == XmlNodeType.Element)
            {
                if (this.IsEmptyElement)
                    return string.Empty;
                if (!Read())
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.XmlInvalidOperation));
        ...
        {
            if (strings == null)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperArgumentNull(nameof(strings));
        ...
        private void CheckArray(Array array, int offset, int count)
        {
            ...
            if (offset > array.Length)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(offset), SR.Format(SR.OffsetExceedsBufferSize, array.Length)));
            if (count < 0)
            ...
            if (count > array.Length - offset)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException(nameof(count), SR.Format(SR.SizeExceedsRemainingBufferSpace, array.Length - offset)));
        }
