...
namespace CoreWCF.Text
{
    internal sealed class Base64Encoding : Encoding
    {
        ...
        public override int GetMaxByteCount(int charCount)
        {
            ...
            if ((charCount % 4) != 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SR.Format(SR.XmlInvalidBase64Length, charCount.ToString())));
        ...
        public unsafe override int GetByteCount(char[] chars, int index, int count)
        {
            ...
            if (count == 0)
                return 0;
            if ((count % 4) != 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SR.Format(SR.XmlInvalidBase64Length, count.ToString())));
                ...
                fixed (byte* _chars = &chars[charIndex])
                {
                    fixed (byte* _bytes = &bytes[byteIndex])
                    {
                        ...
                        while (pch < pchMax)
                        {
                            ...
                            byte pch3 = pch[3];
                            if ((pch0 | pch1 | pch2 | pch3) >= 128)
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(SR.Format(SR.XmlInvalidBase64Sequence, "?", charIndex + (int)(pch - _chars))));
                            // xx765432 xx107654 xx321076 xx543210
                            // 76543210 76543210 76543210
